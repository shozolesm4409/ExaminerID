...
const Remarking: React.FC = () => {
  ...
  const handleApprove = async (data: ExaminerData) => {
    ...
    // Auto-assign Admin Email to 'remarkedBy' if empty
    const currentUserEmail = auth.currentUser?.email || 'Admin';
    const finalRemarkedBy = data.remarkedBy || currentUserEmail;

    // Confirmation
    const confirmMsg = `Confirm Transfer?\n\nStatus: ${targetStatus}\nRM: ${data.rm}\nRemarked By: ${finalRemarkedBy}`;
    ...
    try {
        ...
        // Query the 'examiners' collection, order by 'sl' descending, limit 1
        let newSl = 1;
        try {
            ...
            console.warn("Could not calculate SL automatically (possibly no index). Defaulting to 1 or existing logic. Error:", e);
            // If fetching SL fails (e.g. missing index), we might warn user but still proceed or stop.
            // For now, let's alert user to check console if they want ordered SLs.
        }

        // 1. Create new doc reference in 'examiners'
        // We use the SAME ID to keep it traceable, or we can let firestore generate a new one. 
        // Using same ID is usually better for data integrity.
        ...
        const finalData = { 
            ...
            status: targetStatus,
            rm: data.rm,
            remarkedBy: finalRemarkedBy,
            sl: newSl, // Assign generated SL
